...
export default function ActivatePage() {
  ...
  async function activate() {
    setMessage('')
    setError('')
    if (!petId || !code.trim()) { setError('Selecciona una mascota e ingresa el código.'); return }
    const { data, error } = await supabase.rpc('activate_pet_tag', { p_code: code.trim().toUpperCase(), p_pet_id: petId })
    ...
    <main className="auth-page">
      <section className="auth-card wide">
        ...
        <div className="auth-form">
          ...
          <label>Código del medallón<input value={code} onChange={e => setCode(e.target.value)} placeholder="Ej. PL-22E0A4ED" /></label>
          ...
          <button className="btn primary auth-submit" onClick={activate}>Activar medallón</button>
        </div>
